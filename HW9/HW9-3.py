...
# fill in constraint
for j in range(N):
    m.addConstr(quicksum(xij[i, j] for i in range(N)) <= N * xij[j, j], 'for_j=%s' % j)

# third set of constrint  ----------------------------
# fill in constraint
m.addConstr(quicksum(xij[j, j] for j in range(N)) <= K, 'boundary')
...
for ite in range(LR_MAX_ITE):
    ...
    # fill in obj
    m.setObjective(quicksum(quicksum((cij[i, j] + ui[i]) * xij[i, j] for j in range(N)) for i in range(N)) - quicksum(
        ui[i] for i in range(N)), GRB.MINIMIZE)
